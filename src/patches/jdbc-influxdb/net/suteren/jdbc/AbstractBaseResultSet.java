...
// AbstractBaseResultSet (Apache-2.0) - see pom.xml's patch-influxdb-driver execution
// and this file's getWithClause() below for the actual change.
public abstract class AbstractBaseResultSet implements ResultSet {
	...
	protected static String getWithClause(String tableNamePattern) {
		// Upstream always emitted `FROM "<tableNamePattern>"`, treating it as a single
		// exact measurement name. SQuirreL's DatabaseMetaData.getColumns/getIndexInfo
		// calls pass the SQL LIKE wildcard "%" (meaning "all tables") as that pattern
		// when listing fields/tags across a whole schema, which produced
		// `FROM "%"` - a literal (nonexistent) measurement named "%", so every field/tag
		// lookup silently came back empty. "%" now means "no restriction", matching how
		// GetTablesResultSet.measurementRestriction() already treats it in this same
		// patch set.
		return StringUtils.isNotBlank(tableNamePattern) && !"%".equals(tableNamePattern)
			? String.format(" FROM \"%s\"", quoteName(tableNamePattern)) : "";
	}
}
