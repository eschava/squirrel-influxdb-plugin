...
// Patched copy of net.suteren.jdbc.influxdb:influxdb-jdbc:0.2.6's AbstractProxyResultSet
// (Apache-2.0) - see pom.xml's patch-influxdb-driver execution and isAfterLast()/isLast()
...
public abstract class AbstractProxyResultSet extends AbstractTypeMappingResultSet {
	...
	@Override public boolean isAfterLast() {
		// Upstream used .filter(...).isPresent() here, which is false whenever the current
		// Result has no series at all (Result::getSeries returns null - e.g. a multi-statement
		// query like GetColumnResultSet's "SHOW FIELD KEYS ...; SHOW TAG KEYS ..." where the
		// measurement has no tags, so the TAG KEYS statement's response has no "series" key).
		// That made this condition permanently false once positioned on such a result, so
		// isAfterLast() could never become true and next() looped forever. A missing series
		// list has nothing left to iterate, so it must count as "no more series" (size 0), not
		// silently fail the whole check.
		int seriesCount = influxDbResultSet.getCurrentResult()
			.map(QueryResult.Result::getSeries)
			.map(List::size)
			.orElse(0);
		return influxDbResultSet.getRowPosition().intValue() >= influxDbResultSet.getCurrentRows().size()
			&& influxDbResultSet.getSeriesPosition().intValue() >= seriesCount - 1
	...
	@Override public boolean isLast() {
		// Same fix as isAfterLast() above - a Result with no series at all must count as
		// having 0 series, not silently make this whole check false.
		int seriesCount = influxDbResultSet.getCurrentResult()
			.map(QueryResult.Result::getSeries)
			.map(List::size)
			.orElse(0);
